...
# Path das pastas, com áudios reais / gerados artificialmente
...
def print_summary_stats(label, durations_sec, sizes_bytes):
    """
    Imprime um bloco de sumário formatado para durações e tamanhos de arquivo.
    """
    print(f"\n" + "-"*30)
    ...
    if sizes_bytes:
        sizes_np_mb = np.array(sizes_bytes) / (1024 * 1024) # Convert to MB
        total_size_mb = np.sum(sizes_np_mb)

        if total_size_mb > 1024: # Se for mais de 1 GB, mostre em GB
